...
HOUSE_PARK_RADIUS: int = 2  # world.population park happiness window
PARK_CAPEX: float = 5_000.0  # catalog["park"].capex
SURVEY_INTERVAL_DAYS: int = 56  # 8 weeks
MIN_TREASURY_BUILD: float = 50_000.0
CASHFLOW_RUNWAY_DAYS: int = 28
RESERVE_FLOOR_BUILDOUT: float = 75_000.0
RESERVE_FLOOR_DIVERSIFY: float = 125_000.0
RESERVE_FLOOR_MATURE_LATE: float = 200_000.0
...
class ScriptedAgent(BaseAgent):
    ...
    def act(self, state: dict[str, Any]) -> None:
        ...
        daily_cashflow = _daily_cashflow(state.get("today", {}))
        ...
        cap_critical = capacity <= pop
        jobs_critical = jobs <= pop
        ...
        if (
            ...
            and not (cap_critical or jobs_critical)
            and _can_afford_discretionary(treasury, daily_cashflow, 40_000, phase)
        ...
        if (
            ...
            and not (cap_critical or jobs_critical)
            and _can_afford_discretionary(treasury, daily_cashflow, BATTERY_CAPEX, phase)
        ...
        if (
            ...
            and treasury >= 20_000
            and not (cap_critical or jobs_critical)
            and _can_afford_discretionary(
                treasury,
                daily_cashflow,
                15_000,
                phase,
                allow_negative_cashflow=day - self._last_survey_day >= 2 * SURVEY_INTERVAL_DAYS,
            )
        ):
        ...
        if (
            ...
            and n_prod_wells < 6
            and not (cap_critical or jobs_critical)
            and _can_afford_discretionary(treasury, daily_cashflow, 50_000, phase)
        ):
        ...
        if (
            ...
            and cap_short
            and not nearing_cap
            and _can_afford_growth(treasury, daily_cashflow, 8_000, phase, critical=cap_critical)
        ...
        if want_civilian and jobs_short and treasury >= 8_000:
            ...
            capex = 20_000 if want_industrial else 8_000
            if _can_afford_growth(
                treasury, daily_cashflow, capex, phase, critical=jobs_critical
            ) and self._build_civilian(kind, treasury, cx, cy, w, h, occupied, tiles):
                return
        ...
        if (
            ...
            and treasury >= 1_000
            and _can_afford_growth(
                treasury,
                daily_cashflow,
                500,
                phase,
                critical=cap_critical or jobs_critical,
            )
            and self._extend_roads(
                treasury,
                cx,
                cy,
                w,
        ...
        want_refinery = (
            ...
            and not (cap_critical or jobs_critical)
            and _can_afford_discretionary(treasury, daily_cashflow, 150_000, phase)
        )
        ...
        if (
            ...
            and treasury >= 30_000
            and not (cap_critical or jobs_critical)
            and _can_afford_discretionary(treasury, daily_cashflow, 30_000, phase)
        ):
    ...
    def _extend_roads(
        ...
        *,
        ...
        for x, y in _spiral(cx, cy, w, h):
            ...
            if straight_only:
                extends_network = any(
                    (x - dx, y - dy) in road_set and (x - 2 * dx, y - 2 * dy) in road_set
                    for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1))
                )
            else:
                extends_network = any(
                    (x + dx, y + dy) in road_set
                    for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1))
                )
            if extends_network:
    ...
    def _build_civilian(
        ...
        road_set = {(t["x"], t["y"]) for t in tiles if t["type"] in ("road", "town_hall")}
        park_set = {(t["x"], t["y"]) for t in tiles if t["type"] == "park"}
        first_uncovered_house: tuple[int, int] | None = None
        for x, y in _spiral(cx, cy, w, h):
            if (x, y) in occupied:
                continue
            if any((x + dx, y + dy) in road_set for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1))):
                if tile_type == "house" and not self._has_nearby_park(x, y, park_set):
                    if first_uncovered_house is None:
                        first_uncovered_house = (x, y)
                    continue
                r = self.api.build(tile_type, x, y)
                if r.get("ok"):
                    return True
                ...
                if r.get("error") == "no_road_adjacency":
                    continue
        if tile_type == "house" and first_uncovered_house is not None:
            x, y = first_uncovered_house
            return self._build_park_near(x, y, treasury, occupied, w, h)
        return False

    def _has_nearby_park(self, x: int, y: int, park_set: set[tuple[int, int]]) -> bool:
        """Match the Chebyshev radius used by population happiness."""
        return any(max(abs(x - px), abs(y - py)) <= HOUSE_PARK_RADIUS for px, py in park_set)

    def _build_park_near(
        self,
        house_x: int,
        house_y: int,
        treasury: float,
        ...
        """Place park coverage before adding an uncovered house."""
        if treasury < PARK_CAPEX:
            return False
        for x, y in _spiral(house_x, house_y, w, h):
            if max(abs(x - house_x), abs(y - house_y)) > HOUSE_PARK_RADIUS:
                return False
            if (x, y) == (house_x, house_y) or (x, y) in occupied:
                continue
            r = self.api.build("park", x, y)
            if r.get("ok"):
                return True
            if r.get("error") == "insufficient_funds":
                return False
...
def _daily_cashflow(today: dict[str, Any]) -> float:
    """Net operating cashflow from the exposed per-day ledger."""
    revenue = (
        float(today.get("tax_revenue", 0.0))
        + float(today.get("power_revenue", 0.0))
        + float(today.get("oil_revenue", 0.0))
        + float(today.get("industrial_revenue", 0.0))
        + float(today.get("commercial_revenue", 0.0))
    )
    costs = (
        float(today.get("opex", 0.0))
        + float(today.get("fuel_cost", 0.0))
        + float(today.get("carbon_cost", 0.0))
        + float(today.get("outage_penalty", 0.0))
    )
    return revenue - costs


def _reserve_floor(phase: str) -> float:
    if phase == "buildout":
        return RESERVE_FLOOR_BUILDOUT
    if phase == "diversify":
        return RESERVE_FLOOR_DIVERSIFY
    return RESERVE_FLOOR_MATURE_LATE


def _can_afford_discretionary(
    treasury: float,
    daily_cashflow: float,
    capex: float,
    phase: str,
    *,
    allow_negative_cashflow: bool = False,
) -> bool:
    """Gate speculative capex on reserves and current burn."""
    remaining = treasury - capex
    reserve = _reserve_floor(phase)
    if remaining < reserve:
        return False
    if daily_cashflow >= 0.0:
        return True
    if not allow_negative_cashflow:
        return False
    return remaining >= reserve + abs(daily_cashflow) * CASHFLOW_RUNWAY_DAYS


def _can_afford_growth(
    treasury: float,
    daily_cashflow: float,
    capex: float,
    phase: str,
    *,
    ...
    """Let critical housing/jobs proceed, but pause buffer growth on burn."""
    if critical:
        return treasury >= capex
    return _can_afford_discretionary(treasury, daily_cashflow, capex, phase)
