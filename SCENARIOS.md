...
If you're writing or playing one, you're in the right place. If you want to look up an endpoint, see [API.md](API.md); for the underlying mechanics, see [RULES.md](RULES.md).
...
Source: `world/sim.py:721`, `world/scenario.py:36`.
...
| Key | Effect | Source |
|---|---|---|
| `cloud_factor` | Overrides the smoothed cloud value in `[0.1, 1.0]`. | `world/weather.py:148` |
| `solar_irradiance` | Overrides the per-hour irradiance, post-`cloud × sin`. | `world/weather.py:159` |
| `wind_speed_mps` | Overrides the AR(1) wind value. Below 3 m/s clips wind output to 0. | `world/weather.py:150` |
| `wind_direction_deg` | Overrides the wind heading (cosmetic; turbines auto-yaw). | `world/weather.py:152` |
...
Convention: each entry carries `{"day": day, "kind": "<event_name>", ...}` with whatever extra fields you want surfaced. See [`scenarios/grid_stress.py:79`](scenarios/grid_stress.py) for examples.

## Discovery and attachment

Scenarios are loaded by **dotted path**, matching the agent loader convention:
...
1. **`evaluate.py --scenario <path>`** — attaches before `play_game` runs; the run folder's `metadata.json.scenario` records the dotted path. (slice 06)
2. **`POST /scenario {"dotted_path": "..."}`** — attaches against the running world. `GET /scenario` returns the currently-attached path (or `null` for `NullScenario`).
...
Source: [`world/api.py:140`](world/api.py).
...
`tests/test_determinism.py` pins this contract.
...
Source: [`scenarios/baseline.py`](scenarios/baseline.py).
...
Source: [`scenarios/grid_stress.py`](scenarios/grid_stress.py). Tuning values are class attributes; retune in review without touching `apply`.
...
Source: [`scenarios/economy_stress.py`](scenarios/economy_stress.py).

## Authoring a new scenario

1. **One file per scenario.** Drop `scenarios/<your_slug>.py`. The module docstring should name the stress profile in one paragraph and list the levers it pulls.
2. **Subclass `Scenario`.** Override `apply(world, day)` and pin `seed`. Use class attributes for tuning values so a maintainer can retune in review without touching `apply` (see [`scenarios/grid_stress.py:36`](scenarios/grid_stress.py)).
3. **Stay on the override taxonomy.** Weather overrides, mutable pricing fields, named-event injections, scenario trace. Anything else breaks the determinism contract.
...
5. **Write a regression test.** Add `scenarios/tests/test_<your_slug>.py` that drives the world a few days and asserts overrides fire on the documented days. The grid_stress test (`scenarios/tests/test_grid_stress.py`) is a worked example.
6. **Verify locally.** `make check` must pass.
